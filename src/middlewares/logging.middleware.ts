...
import { sendTelegramMessage } from '@/service/core/notifier.service.js';
...
export const createLogger = (env) => {
  ...
  if (env === APPLICATION_ENV_TYPES.PROD || env === APPLICATION_ENV_TYPES.TEST) {
    transports.push(
      // Error logs
...
export const errorLoggingMiddleware = (logger) => (error, req, res, next) => {
  ...
  if (error.statusCode >= 500 && config.ENV === APPLICATION_ENV_TYPES.PROD) {
    sendTelegramMessage({
      message: error.message,
      stack: error.stack,
      method: req.method,
...
export const securityLoggingMiddleware = (logger) => (req, res, next) => {
  ...
  if (req.headers['x-forwarded-for'] && req.headers['x-forwarded-for'].split(',').length > 1) {
  ...
  if (userAgent && userAgent.includes('python') && config.ENV === APPLICATION_ENV_TYPES.PROD) {
